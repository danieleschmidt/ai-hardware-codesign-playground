...
try:
    ...
    IsolationForest = RandomForestClassifier = None
    StandardScaler = PCA = DBSCAN = None
...
class MetricSeries:
    ...
    def calculate_statistics(self, window_minutes: int = 60) -> Dict[str, float]:
        ...
        return {
            ...
            'p95': np.percentile(recent_values, 95) if np is not None else sorted(recent_values)[int(0.95 * len(recent_values))],
            'p99': np.percentile(recent_values, 99) if np is not None else sorted(recent_values)[int(0.99 * len(recent_values))]
        }
...
class AnomalyDetector:
    ...
    def __init__(self, contamination: float = 0.1, window_size: int = 100):
        ...
        if IsolationForest is not None and StandardScaler is not None:
            ...
            logger.warning("sklearn not available - anomaly detection will use statistical methods only")
